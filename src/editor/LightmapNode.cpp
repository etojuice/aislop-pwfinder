...
bool LightmapNode::insert(int iw, int ih, int& outX, int& outY)
{
    ...
    {
        child[0] = std::make_unique<LightmapNode>(x, y, iw, h);
        child[1] = std::make_unique<LightmapNode>(x + iw, y, w - iw, h);
    }
    else // split horizontally
    {
        child[0] = std::make_unique<LightmapNode>(x, y, w, ih);
        child[1] = std::make_unique<LightmapNode>(x, y + ih, w, h - ih);
    }
