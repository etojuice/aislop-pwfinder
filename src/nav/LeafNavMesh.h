...
struct LeafLink {
	int node; // which leaf is linked to
...
struct LeafNode {
	std::vector<LeafLink> links;
	int id;
	vec3 origin; // the best position for pathing (not necessarily the center)
	int entidx; // 0 for world leaves, else an entity leaf which may be relocated, enabled, or disabled

	// for debugging
	vec3 center;
	vec3 mins, maxs; // for octree insertion, not needed after generation
	std::vector<Polygon3D> leafFaces;
...
public:
	...
	std::vector<int> AStarRoute(int startNodeIdx, int endNodeIdx);

	std::vector<int> dijkstraRoute(int start, int end);
