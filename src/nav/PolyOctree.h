...
public:
    ...
    void getPolysInRegion(Polygon3D* poly, std::vector<bool>& regionPolys);

private:
    void buildOctree(PolyOctant* node, int currentDepth);

    void getPolysInRegion(PolyOctant* node, Polygon3D* poly, int currentDepth, std::vector<bool>& regionPolys);
